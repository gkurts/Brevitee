...
namespace Brevitee.Data
{
    public class InsertFormat: SetFormat
    {
        ...
        {
            AssignNumbers();
			string columns = this.Parameters.ToArray().ToDelimited(p => ColumnNameFormatter(p.ColumnName));//string columns = this.Parameters.ToArray().ToDelimited(p => string.Format("[{0}]", p.ColumnName));
