...
﻿
	public class BuildJobQueryContext
	{
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.Where(where, db);
			}
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.Where(where, orderBy, db);
			}

			public BuildJob OneWhere(WhereDelegate<BuildJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.OneWhere(where, db);
			}
		
			public BuildJob FirstOneWhere(WhereDelegate<BuildJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.FirstOneWhere(where, db);
			}

			public BuildJobCollection Top(int count, WhereDelegate<BuildJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.Top(count, where, db);
			}
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.Top(count, where, orderBy, db);
			}

			public long Count(WhereDelegate<BuildJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildJob.Count(where, db);
			}
	}
	...
	public class BuildResultQueryContext
	{
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.Where(where, db);
			}
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.Where(where, orderBy, db);
			}

			public BuildResult OneWhere(WhereDelegate<BuildResultColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.OneWhere(where, db);
			}
		
			public BuildResult FirstOneWhere(WhereDelegate<BuildResultColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.FirstOneWhere(where, db);
			}

			public BuildResultCollection Top(int count, WhereDelegate<BuildResultColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.Top(count, where, db);
			}
			...
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.Top(count, where, orderBy, db);
			}

			public long Count(WhereDelegate<BuildResultColumns> where, Database db = null)
			{
				return Brevitee.Automation.ContinuousIntegration.Data.BuildResult.Count(where, db);
			}
	}
