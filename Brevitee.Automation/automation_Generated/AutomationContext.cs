...
﻿
	public class DeferredJobQueryContext
	{
			...
			{
				return Brevitee.Automation.Data.DeferredJob.Where(where, db);
			}
			...
			{
				return Brevitee.Automation.Data.DeferredJob.Top(count, where, orderBy, db);
			}

			public long Count(WhereDelegate<DeferredJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.Data.DeferredJob.Count(where, db);
			}
	}
	...
	public class RunningJobQueryContext
	{
			...
			{
				return Brevitee.Automation.Data.RunningJob.Where(where, db);
			}
			...
			{
				return Brevitee.Automation.Data.RunningJob.Top(count, where, orderBy, db);
			}

			public long Count(WhereDelegate<RunningJobColumns> where, Database db = null)
			{
				return Brevitee.Automation.Data.RunningJob.Count(where, db);
			}
	}
