...
namespace Brevitee.Data.Integration.Tests
{
	...
	public class DaoCrudTests: CommandLineTestInterface
	{
		...
		public void PropertyTypeTest()
		{
			...
			{
				OutLineFormat("{0}.{1}: {2}", ConsoleColor.DarkYellow, this.GetType().Name, methodName, db.GetType().Name);
				DaoReferenceObject obj = new DaoReferenceObject();
				obj.IntProperty = RandomNumber.Between(0, 100);
				obj.DecimalProperty = 10.15m;
				obj.LongProperty = 99999;
				//obj.DateTimeProperty = DateTime.UtcNow; // DateTime is broken in SQLite
				obj.BoolProperty = RandomHelper.Bool();
				...
				//Expect.AreEqual(obj.DateTimeProperty, retrieved.DateTimeProperty, "DateTimeProperty didn't match");
		...
		public void ChildListTest()
		{
			...
			{
				OutLineFormat("{0}.{1}: {2}", ConsoleColor.DarkYellow, this.GetType().Name, methodName, db.GetType().Name);
				TestTable parent = new TestTable();
		...
		public void XrefListTest()
		{
			...
			_testDatabases.Each(db =>
			{
				...
				4.Times(i =>
				{
					lefts.Add(Tools.CreateLeft("Left ".RandomLetters(6), db));
				});
				List<Right> rights = new List<Right>();
				3.Times(i =>
				{
					rights.Add(Tools.CreateRight("Right ".RandomLetters(5), db));
				});
