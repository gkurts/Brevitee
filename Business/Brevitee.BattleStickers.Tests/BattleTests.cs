...
namespace Brevitee.BattleStickers.Business.Data.Tests
{
    ...
    public class BattleTests: CommandLineTestInterface
    {
        ...
        private void BattleLoop(Battle battle)
        {
            ...
            {
                OutLine(l, ConsoleColor.Green);
            };
            battle.TurnEnded += (b) =>
            {
                OutLineFormat("Battle Multiplier: ({0})"._Format(battle.Multiplier));
            ...
            OutLineFormat("{0}", ConsoleColor.Green, battle.State.ToString());
            OutLineFormat("\tTurn Count: {0}", ConsoleColor.Yellow, turnCount);
        }
        ...
        private void ShowCharacters(PlayerField field, ConsoleColor color)
        {
            ...
            {
                OutLineFormat("{0}: ({1}), D: ({2}), S: ({3}), M: ({4}), A: ({5}), MH: ({6})",
