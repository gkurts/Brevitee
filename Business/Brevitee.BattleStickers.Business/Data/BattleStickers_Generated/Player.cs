...
namespace Brevitee.BattleStickers.Services.Data
{
	...
	public partial class Player: Dao
	{
		...
		{
﻿
            this.ChildCollections.Add("Battle_RockPaperScissorsWinnerId", new BattleCollection(Database.GetQuery<BattleColumns, Battle>((c) => c.RockPaperScissorsWinnerId == this.Id), this, "RockPaperScissorsWinnerId"));	﻿
            this.ChildCollections.Add("PlayerOne_PlayerId", new PlayerOneCollection(Database.GetQuery<PlayerOneColumns, PlayerOne>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerTwo_PlayerId", new PlayerTwoCollection(Database.GetQuery<PlayerTwoColumns, PlayerTwo>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerCharacter_PlayerId", new PlayerCharacterCollection(Database.GetQuery<PlayerCharacterColumns, PlayerCharacter>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerWeapon_PlayerId", new PlayerWeaponCollection(Database.GetQuery<PlayerWeaponColumns, PlayerWeapon>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerSpell_PlayerId", new PlayerSpellCollection(Database.GetQuery<PlayerSpellColumns, PlayerSpell>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerSkill_PlayerId", new PlayerSkillCollection(Database.GetQuery<PlayerSkillColumns, PlayerSkill>((c) => c.PlayerId == this.Id), this, "PlayerId"));	﻿
            this.ChildCollections.Add("PlayerEquipment_PlayerId", new PlayerEquipmentCollection(Database.GetQuery<PlayerEquipmentColumns, PlayerEquipment>((c) => c.PlayerId == this.Id), this, "PlayerId"));				﻿
...
﻿	// property:Level, columnName:Level	
	[Brevitee.Data.Column(Name="Level", DbDataType="Int", MaxLength="10", AllowNull=false)]
	public int? Level
	{
		get
		{
...
﻿	// property:ExperiencePoints, columnName:ExperiencePoints	
	[Brevitee.Data.Column(Name="ExperiencePoints", DbDataType="Int", MaxLength="10", AllowNull=false)]
...
﻿
		...
		public static PlayerCollection LoadAll(Database database = null)
		{
			...
			sql.Select<Player>();
			Database db = database ?? Db.For<Player>();
			var results = new PlayerCollection(sql.GetDataTable(db));
			results.Database = db;
			return results;
		}

		public static Player GetById(int id, Database database = null)
		{
			return GetById((long)id, database);
		}

		public static Player GetById(long id, Database database = null)
		{
			return OneWhere(c => c.KeyColumn == id, database);
		}

		public static PlayerCollection Query(QueryFilter filter, Database database = null)
		{
			return Where(filter, database);
		}
				
		public static PlayerCollection Where(QueryFilter filter, Database database = null)
		{
			WhereDelegate<PlayerColumns> whereDelegate = (c) => filter;
		...
		/// </param>
		/// <param name="db"></param>
		public static PlayerCollection Where(Func<PlayerColumns, QueryFilter<PlayerColumns>> where, OrderBy<PlayerColumns> orderBy = null, Database database = null)
		{
			database = database ?? Db.For<Player>();
			return new PlayerCollection(database.GetQuery<PlayerColumns, Player>(where, orderBy), true);
		}
		...
		/// </param>
		/// <param name="db"></param>
		public static PlayerCollection Where(WhereDelegate<PlayerColumns> where, Database database = null)
		{		
			database = database ?? Db.For<Player>();
			var results = new PlayerCollection(database, database.GetQuery<PlayerColumns, Player>(where), true);
			return results;
		}
		...
		public static PlayerCollection Where(WhereDelegate<PlayerColumns> where, OrderBy<PlayerColumns> orderBy = null, Database database = null)
		{		
			database = database ?? Db.For<Player>();
			var results = new PlayerCollection(database, database.GetQuery<PlayerColumns, Player>(where, orderBy), true);
			return results;
		}
		...
		/// <param name="database"></param>
		public static PlayerCollection Where(QiQuery where, Database database = null)
		{
			var results = new PlayerCollection(database, Select<PlayerColumns>.From<Player>().Where(where, database));
			return results;
		}
		...
		/// </summary>
		public static Player GetOneWhere(QueryFilter where, Database database = null)
		{
		...
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Player OneWhere(QueryFilter where, Database database = null)
		{
			WhereDelegate<PlayerColumns> whereDelegate = (c) => where;
		...
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Player GetOneWhere(WhereDelegate<PlayerColumns> where, Database database = null)
		{
			var result = OneWhere(where, database);
			if(result == null)
			{
		...
		/// </param>
		/// <param name="database"></param>
		public static Player OneWhere(WhereDelegate<PlayerColumns> where, Database database = null)
		{
		...
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Player OneWhere(QiQuery where, Database database = null)
		{
		...
		/// </param>
		/// <param name="database"></param>
		public static Player FirstOneWhere(WhereDelegate<PlayerColumns> where, Database database = null)
		{
			var results = Top(1, where, database);
			if(results.Count > 0)
			{
		...
		/// </param>
		/// <param name="database"></param>
		public static Player FirstOneWhere(WhereDelegate<PlayerColumns> where, OrderBy<PlayerColumns> orderBy, Database database = null)
		{
		...
		/// </param>
		/// <param name="database"></param>
		public static Player FirstOneWhere(QueryFilter where, OrderBy<PlayerColumns> orderBy = null, Database database = null)
		{
			WhereDelegate<PlayerColumns> whereDelegate = (c) => where;
		...
		/// </param>
		/// <param name="database"></param>
		public static PlayerCollection Top(int count, WhereDelegate<PlayerColumns> where, Database database = null)
		{
		...
		/// </param>
		/// <param name="db"></param>
		public static PlayerCollection Top(int count, QueryFilter where, OrderBy<PlayerColumns> orderBy = null, Database database = null)
		{
			Database db = database ?? Db.For<Player>();
			QuerySet query = GetQuerySet(db);
			query.Top<Player>(count);
		...
		/// </param>
		/// <param name="db"></param>
		public static PlayerCollection Top(int count, QiQuery where, Database database = null)
		{
			Database db = database ?? Db.For<Player>();
			QuerySet query = GetQuerySet(db);
			query.Top<Player>(count);
		...
		private static Player CreateFromFilter(IQueryFilter filter, Database database = null)
		{
			Database db = database ?? Db.For<Player>();			
			var dao = new Player();
