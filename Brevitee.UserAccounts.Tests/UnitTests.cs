...
namespace Brevitee.UserAccounts.Tests
{
    public partial class UserAccountTestsProgram
    {
        ...
        public void SignUpShouldCreateUserEntry()
        {
            string userName = MethodBase.GetCurrentMethod().Name;
            UserManager userProxy = new UserManager();
            userProxy.SignUp("test@stickerize.me", userName, "password".Sha1(), false);
        ...
        public void SignUpShouldCreateAccountEntry()
        {
            string userName = MethodBase.GetCurrentMethod().Name;
            UserManager userProxy = new UserManager();
            userProxy.SignUp("test@domain.cxm", userName, "password".Sha1(), false);
        ...
        public void SignUpShouldSavePasswordHashOfHash()
        {
            ...
            userProxy.SignUp("test@domain.cxm", userName, passwordHash, false);
        ...
        public void LoginShouldNotReturnNull()
        {
            ...
            userProxy.SignUp("test@domain.cxm", userName, passHash, false);
            LoginResponse result = userProxy.Login(userName, passHash);
            Expect.IsNotNull(result);
        }

        [UnitTest]
        public void UserOfCurrentSessionShouldNeverBeNull()
        {
            UserManager userProxy = new UserManager();
            ...
            Expect.IsNotNull(session.UserOfUserId);
        }
        ...
        [UnitTest]
        public void ThereShouldBeNoSessionsBecauseTheyGetDeletedByTheStaticInitializer()
        {
            SessionCollection sessions = Session.LoadAll();
            Expect.AreEqual(0, sessions.Count);
        }
        ...
        [UnitTest]
        public void ShouldBeSameUsers()
        {
            User anon1 = User.Anonymous;
            User anon2 = User.OneWhere(c => c.Id == anon1.Id);
            Expect.AreEqual(anon1, anon2, "not equal");
            Expect.IsTrue(anon1.Equals(anon2), "not DotEquals");
        }
        ...
        public void ShouldInitializeDaoUserManagerAppNameResolver()
        {
            ...
            Expect.IsNotNull(mgr);
            Expect.IsNotNull(mgr.ApplicationNameResolver);
            Expect.IsObjectOfType<TestAppNameResolver>(mgr.ApplicationNameResolver, "Resolver was a {0}"._Format(mgr.ApplicationNameResolver.GetType().Name));
        }
