// Model is Table
using System;
using System.Data;
using System.Data.Common;
using Brevitee;
using Brevitee.Data;
using Brevitee.Data.Qi;

namespace Brevitee.Shop
{
	// schema = Shop
	// connection Name = Shop
	[Serializable]
	[Brevitee.Data.Table("Currency", "Shop")]
	public partial class Currency: Dao
	{
		public Currency():base()
		{
			this.SetKeyColumnName();
			this.SetChildren();
		}

		public Currency(DataRow data): base(data)
		{
			this.SetKeyColumnName();
			this.SetChildren();
		}

		public static implicit operator Currency(DataRow data)
		{
			return new Currency(data);
		}

		private void SetChildren()
		{
﻿
            this.ChildCollections.Add("CurrencyCountry_CurrencyId", new CurrencyCountryCollection(Database.GetQuery<CurrencyCountryColumns, CurrencyCountry>((c) => c.CurrencyId == this.Id), this, "CurrencyId"));	﻿
            this.ChildCollections.Add("Price_CurrencyId", new PriceCollection(Database.GetQuery<PriceColumns, Price>((c) => c.CurrencyId == this.Id), this, "CurrencyId"));							
		}

﻿	// property:Id, columnName:Id	
	[Exclude]
	[Brevitee.Data.KeyColumn(Name="Id", DbDataType="BigInt", MaxLength="19")]
	public long? Id
	{
		get
		{
			return GetLongValue("Id");
		}
		set
		{
			SetValue("Id", value);
		}
	}

﻿	// property:Uuid, columnName:Uuid	
	[Brevitee.Data.Column(Name="Uuid", DbDataType="VarChar", MaxLength="4000", AllowNull=false)]
	public string Uuid
	{
		get
		{
			return GetStringValue("Uuid");
		}
		set
		{
			SetValue("Uuid", value);
		}
	}

﻿	// property:Symbol, columnName:Symbol	
	[Brevitee.Data.Column(Name="Symbol", DbDataType="VarChar", MaxLength="4000", AllowNull=false)]
	public string Symbol
	{
		get
		{
			return GetStringValue("Symbol");
		}
		set
		{
			SetValue("Symbol", value);
		}
	}

﻿	// property:Name, columnName:Name	
	[Brevitee.Data.Column(Name="Name", DbDataType="VarChar", MaxLength="4000", AllowNull=false)]
	public string Name
	{
		get
		{
			return GetStringValue("Name");
		}
		set
		{
			SetValue("Name", value);
		}
	}



				
﻿
	[Exclude]	
	public CurrencyCountryCollection CurrencyCountriesByCurrencyId
	{
		get
		{
			if (this.IsNew)
			{
				throw new InvalidOperationException("The current instance of type({0}) hasn't been saved and will have no child collections, call Save() or Save(Database) first."._Format(this.GetType().Name));
			}

			if(!this.ChildCollections.ContainsKey("CurrencyCountry_CurrencyId"))
			{
				SetChildren();
			}

			var c = (CurrencyCountryCollection)this.ChildCollections["CurrencyCountry_CurrencyId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
			return c;
		}
	}
	﻿
	[Exclude]	
	public PriceCollection PricesByCurrencyId
	{
		get
		{
			if (this.IsNew)
			{
				throw new InvalidOperationException("The current instance of type({0}) hasn't been saved and will have no child collections, call Save() or Save(Database) first."._Format(this.GetType().Name));
			}

			if(!this.ChildCollections.ContainsKey("Price_CurrencyId"))
			{
				SetChildren();
			}

			var c = (PriceCollection)this.ChildCollections["Price_CurrencyId"];
			if(!c.Loaded)
			{
				c.Load(Database);
			}
			return c;
		}
	}
			

		/// <summary>
		/// Gets a query filter that should uniquely identify
		/// the current instance.  The default implementation
		/// compares the Id/key field to the current instance's.
		/// </summary> 
		public override IQueryFilter GetUniqueFilter()
		{
			var colFilter = new CurrencyColumns();
			return (colFilter.KeyColumn == IdValue);
		}
		/// <summary>
		/// Return every record in the Currency table.
		/// </summary>
		/// <param name="database">
		/// The database to load from or null
		/// </param>
		public static CurrencyCollection LoadAll(Database database = null)
		{
			SqlStringBuilder sql = new SqlStringBuilder();
			sql.Select<Currency>();
			Database db = database ?? Db.For<Currency>();
			var results = new CurrencyCollection(sql.GetDataTable(db));
			results.Database = db;
			return results;
		}

		public static Currency GetById(int id, Database database = null)
		{
			return GetById((long)id, database);
		}

		public static Currency GetById(long id, Database database = null)
		{
			return OneWhere(c => c.KeyColumn == id, database);
		}

		public static CurrencyCollection Query(QueryFilter filter, Database database = null)
		{
			return Where(filter, database);
		}
				
		public static CurrencyCollection Where(QueryFilter filter, Database database = null)
		{
			WhereDelegate<CurrencyColumns> whereDelegate = (c) => filter;
			return Where(whereDelegate, database);
		}

		/// <summary>
		/// Execute a query and return the results. 
		/// </summary>
		/// <param name="where">A Func delegate that recieves a CurrencyColumns 
		/// and returns a QueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="db"></param>
		public static CurrencyCollection Where(Func<CurrencyColumns, QueryFilter<CurrencyColumns>> where, OrderBy<CurrencyColumns> orderBy = null, Database database = null)
		{
			database = database ?? Db.For<Currency>();
			return new CurrencyCollection(database.GetQuery<CurrencyColumns, Currency>(where, orderBy), true);
		}
		
		/// <summary>
		/// Execute a query and return the results. 
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="db"></param>
		public static CurrencyCollection Where(WhereDelegate<CurrencyColumns> where, Database database = null)
		{		
			database = database ?? Db.For<Currency>();
			var results = new CurrencyCollection(database, database.GetQuery<CurrencyColumns, Currency>(where), true);
			return results;
		}
		   
		/// <summary>
		/// Execute a query and return the results. 
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="orderBy">
		/// Specifies what column and direction to order the results by
		/// </param>
		/// <param name="database"></param>
		public static CurrencyCollection Where(WhereDelegate<CurrencyColumns> where, OrderBy<CurrencyColumns> orderBy = null, Database database = null)
		{		
			database = database ?? Db.For<Currency>();
			var results = new CurrencyCollection(database, database.GetQuery<CurrencyColumns, Currency>(where, orderBy), true);
			return results;
		}

		/// <summary>
		/// This method is intended to respond to client side Qi queries.
		/// Use of this method from .Net should be avoided in favor of 
		/// one of the methods that take a delegate of type
		/// WhereDelegate<CurrencyColumns>.
		/// </summary>
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static CurrencyCollection Where(QiQuery where, Database database = null)
		{
			var results = new CurrencyCollection(database, Select<CurrencyColumns>.From<Currency>().Where(where, database));
			return results;
		}
				
		/// <summary>
		/// Get one entry matching the specified filter.  If none exists 
		/// one will be created; success will depend on the nullability
		/// of the specified columns.
		/// </summary>
		public static Currency GetOneWhere(QueryFilter where, Database database = null)
		{
			var result = OneWhere(where, database);
			if(result == null)
			{
				result = CreateFromFilter(where, database);
			}

			return result;
		}

		/// <summary>
		/// Execute a query that should return only one result.  If more
		/// than one result is returned a MultipleEntriesFoundException will 
		/// be thrown.  
		/// </summary>
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Currency OneWhere(QueryFilter where, Database database = null)
		{
			WhereDelegate<CurrencyColumns> whereDelegate = (c) => where;
			var result = Top(1, whereDelegate, database);
			return OneOrThrow(result);
		}

		/// <summary>
		/// Get one entry matching the specified filter.  If none exists 
		/// one will be created; success will depend on the nullability
		/// of the specified columns.
		/// </summary>
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Currency GetOneWhere(WhereDelegate<CurrencyColumns> where, Database database = null)
		{
			var result = OneWhere(where, database);
			if(result == null)
			{
				CurrencyColumns c = new CurrencyColumns();
				IQueryFilter filter = where(c); 
				result = CreateFromFilter(filter, database);
			}

			return result;
		}

		/// <summary>
		/// Execute a query that should return only one result.  If more
		/// than one result is returned a MultipleEntriesFoundException will 
		/// be thrown.  This method is most commonly used to retrieve a
		/// single Currency instance by its Id/Key value
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="database"></param>
		public static Currency OneWhere(WhereDelegate<CurrencyColumns> where, Database database = null)
		{
			var result = Top(1, where, database);
			return OneOrThrow(result);
		}
					 
		/// <summary>
		/// This method is intended to respond to client side Qi queries.
		/// Use of this method from .Net should be avoided in favor of 
		/// one of the methods that take a delegate of type
		/// WhereDelegate<CurrencyColumns>.
		/// </summary>
		/// <param name="where"></param>
		/// <param name="database"></param>
		public static Currency OneWhere(QiQuery where, Database database = null)
		{
			var results = Top(1, where, database);
			return OneOrThrow(results);
		}

		/// <summary>
		/// Execute a query and return the first result.  This method will issue a sql TOP clause so only the 
		/// specified number of values will be returned.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="database"></param>
		public static Currency FirstOneWhere(WhereDelegate<CurrencyColumns> where, Database database = null)
		{
			var results = Top(1, where, database);
			if(results.Count > 0)
			{
				return results[0];
			}
			else
			{
				return null;
			}
		}
		
		/// <summary>
		/// Execute a query and return the first result.  This method will issue a sql TOP clause so only the 
		/// specified number of values will be returned.
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="database"></param>
		public static Currency FirstOneWhere(WhereDelegate<CurrencyColumns> where, OrderBy<CurrencyColumns> orderBy, Database database = null)
		{
			var results = Top(1, where, orderBy, database);
			if(results.Count > 0)
			{
				return results[0];
			}
			else
			{
				return null;
			}
		}

		/// <summary>
		/// Shortcut for Top(1, where, orderBy, database)
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="database"></param>
		public static Currency FirstOneWhere(QueryFilter where, OrderBy<CurrencyColumns> orderBy = null, Database database = null)
		{
			WhereDelegate<CurrencyColumns> whereDelegate = (c) => where;
			var results = Top(1, whereDelegate, orderBy, database);
			if(results.Count > 0)
			{
				return results[0];
			}
			else
			{
				return null;
			}
		}

		/// <summary>
		/// Execute a query and return the specified number
		/// of values. This method will issue a sql TOP clause so only the 
		/// specified number of values will be returned.
		/// </summary>
		/// <param name="count">The number of values to return.
		/// This value is used in the sql query so no more than this 
		/// number of values will be returned by the database.
		/// </param>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="database"></param>
		public static CurrencyCollection Top(int count, WhereDelegate<CurrencyColumns> where, Database database = null)
		{
			return Top(count, where, null, database);
		}

		/// <summary>
		/// Execute a query and return the specified number of values.  This method
		/// will issue a sql TOP clause so only the specified number of values
		/// will be returned.
		/// of values
		/// </summary>
		/// <param name="count">The number of values to return.
		/// This value is used in the sql query so no more than this 
		/// number of values will be returned by the database.
		/// </param>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="orderBy">
		/// Specifies what column and direction to order the results by
		/// </param>
		/// <param name="database"></param>
		public static CurrencyCollection Top(int count, WhereDelegate<CurrencyColumns> where, OrderBy<CurrencyColumns> orderBy, Database database = null)
		{
			CurrencyColumns c = new CurrencyColumns();
			IQueryFilter filter = where(c);         
			
			Database db = database ?? Db.For<Currency>();
			QuerySet query = GetQuerySet(db); 
			query.Top<Currency>(count);
			query.Where(filter);

			if(orderBy != null)
			{
				query.OrderBy<CurrencyColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<CurrencyCollection>(0);
			results.Database = db;
			return results;
		}

		/// <summary>
		/// Execute a query and return the specified number of values.  This method
		/// will issue a sql TOP clause so only the specified number of values
		/// will be returned.
		/// of values
		/// </summary>
		/// <param name="count">The number of values to return.
		/// This value is used in the sql query so no more than this 
		/// number of values will be returned by the database.
		/// </param>
		/// <param name="where">A QueryFilter used to filter the 
		/// results
		/// </param>
		/// <param name="orderBy">
		/// Specifies what column and direction to order the results by
		/// </param>
		/// <param name="db"></param>
		public static CurrencyCollection Top(int count, QueryFilter where, OrderBy<CurrencyColumns> orderBy = null, Database database = null)
		{
			Database db = database ?? Db.For<Currency>();
			QuerySet query = GetQuerySet(db);
			query.Top<Currency>(count);
			query.Where(where);

			if(orderBy != null)
			{
				query.OrderBy<CurrencyColumns>(orderBy);
			}

			query.Execute(db);
			var results = query.Results.As<CurrencyCollection>(0);
			results.Database = db;
			return results;
		}

		/// <summary>
		/// Execute a query and return the specified number of values.  This method
		/// will issue a sql TOP clause so only the specified number of values
		/// will be returned.
		/// of values
		/// </summary>
		/// <param name="count">The number of values to return.
		/// This value is used in the sql query so no more than this 
		/// number of values will be returned by the database.
		/// </param>
		/// <param name="where">A QueryFilter used to filter the 
		/// results
		/// </param>
		/// <param name="orderBy">
		/// Specifies what column and direction to order the results by
		/// </param>
		/// <param name="db"></param>
		public static CurrencyCollection Top(int count, QiQuery where, Database database = null)
		{
			Database db = database ?? Db.For<Currency>();
			QuerySet query = GetQuerySet(db);
			query.Top<Currency>(count);
			query.Where(where);
			query.Execute(db);
			var results = query.Results.As<CurrencyCollection>(0);
			results.Database = db;
			return results;
		}

		/// <summary>
		/// Execute a query and return the number of results
		/// </summary>
		/// <param name="where">A WhereDelegate that recieves a CurrencyColumns 
		/// and returns a IQueryFilter which is the result of any comparisons
		/// between CurrencyColumns and other values
		/// </param>
		/// <param name="db"></param>
		public static long Count(WhereDelegate<CurrencyColumns> where, Database database = null)
		{
			CurrencyColumns c = new CurrencyColumns();
			IQueryFilter filter = where(c) ;

			Database db = database ?? Db.For<Currency>();
			QuerySet query = GetQuerySet(db);	 
			query.Count<Currency>();
			query.Where(filter);	  
			query.Execute(db);
			return query.Results.As<CountResult>(0).Value;
		}

		private static Currency CreateFromFilter(IQueryFilter filter, Database database = null)
		{
			Database db = database ?? Db.For<Currency>();			
			var dao = new Currency();
			filter.Parameters.Each(p=>
			{
				dao.Property(p.ColumnName, p.Value);
			});
			dao.Save(db);
			return dao;
		}
		
		private static Currency OneOrThrow(CurrencyCollection c)
		{
			if(c.Count == 1)
			{
				return c[0];
			}
			else if(c.Count > 1)
			{
				throw new MultipleEntriesFoundException();
			}

			return null;
		}

	}
}																								
